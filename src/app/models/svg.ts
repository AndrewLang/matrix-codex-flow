...
export class Svgs {
  private static readonly icons = new Map<string, SvgIcon>(
    [
      ['empty', {
        name: 'empty',
        title: 'Empty',
        tags: ['blank', 'none', 'placeholder'],
        svg: `
      ...
      ['vscode', {
        ...
        tags: ['editor', 'ide', 'microsoft', 'dev'],
        svg: `
        <svg class="block w-full h-full " viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  static get(name: string): SvgIcon | null {
    if (!name) return null;
    return this.icons.get(name) ?? null;
  }

  static empty(): SvgIcon {
  ...
  static register(icon: SvgIcon): void {
    if (!icon?.name?.trim())
      throw new Error('SvgIcon.name is required');
    if (!icon?.svg?.includes('<svg'))
      throw new Error(`SvgIcon.svg must contain <svg> markup (name=${icon.name})`);
